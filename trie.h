...
private:
    T data;
    bool last;        //if this is the last
    trienode<T> *next;//next character
    trienode<T> *follow;//follow
};
...
private:
    ...
    //newly-defined hash function
    //Basically it converts a string to another with exactly one character(at index diff) in difference
    AList<T> copy(trienode<T>* pt, AList<T> list, int diff){
        AList<T> newlist;
        for (int i = 1; i <= list.getlength(); i++)
        {
            //except for this index, the others remain the same from the original
            if(i==diff+1){
                newlist.append(pt->getdata());
                continue;
            }
